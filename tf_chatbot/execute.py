...
# require to adjust for different datasets
_buckets = [(5, 10), (15, 20), (25, 40), (35, 70), (75, 110), (125, 160)]
...
def read_data(source_path, target_path, max_size=None):
  ...
  # each entry corresponds to the list of one bucket size   
  # Shape: [bucket_id, source, target]
  ...
  with tf.gfile.GFile(source_path, mode="r") as source_file:
    with tf.gfile.GFile(target_path, mode="r") as target_file:
      ...
      while source and target and (not max_size or counter < max_size):
        # process each line of source and target   
        counter += 1
        if counter % 100000 == 0:
...
#   Shape: [bucket_id]    
    ...
    # in each iterataion, data from one bucket is used to train 
    while True:
      ...
      if current_step % gConfig['steps_per_checkpoint'] == 0:
        ...
        for bucket_id in xrange(len(_buckets)):
          ...
          # Shape: [length in the bucket, batch_size]
...
def decode_test():
  ...
  with tf.Session() as sess:
    ...
    while sentence:
      ...
      token_ids = data_utils.sentence_to_token_ids(tf.compat.as_bytes(sentence).decode('utf-8'), enc_vocab)
